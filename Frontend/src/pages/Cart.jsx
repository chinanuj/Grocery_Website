...
const Cart = ({ onNavigate }) => {
  ...
  const fetchCart = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('http://localhost:5000/api/cart');
  ...
  const fetchProductDetails = async (cartItems) => {
    try {
      ...
      for (const item of cartItems) {
        const productResponse = await api.get(`http://localhost:5000/api/products/${item.product_id._id}`);
  ...
  const removeFromCart = async (productId) => {
    try {
      const response = await api.delete(`http://localhost:5000/api/cart/${productId}`);
  ...
  const placeOrder = async () => {
    try {
      const response = await api.post('http://localhost:5000/api/orders');
      setAlert({ type: 'success', message: response.data.message });
      setCart([]);
    } catch (error) {
      setAlert({
